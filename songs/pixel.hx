...
function create(){
	// if(PlayState.instance.curSong != "ur-girl"){
...
function onNoteCreation(event) {
	...
	if (event.note.isSustainNote) {
		note.loadGraphic(Paths.image('stages/weeb/pixelUI/arrowEnds'), true, 7, 6);
		note.animation.add("hold", [event.strumID]);
		note.animation.add("holdend", [4 + event.strumID]);
	} else {
		note.loadGraphic(Paths.image('stages/weeb/pixelUI/arrows-pixels'), true, 17, 17);
...
function onStrumCreation(event) {
	...
	strum.loadGraphic(Paths.image('stages/weeb/pixelUI/arrows-pixels'), true, 17, 17);
